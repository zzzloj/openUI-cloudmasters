...
class public_forums_extras_vote extends ipsCommand
{
	...
	*/
	...
	public function topicAddVoteToPoll()
	{
		...
		$this->registry->output->redirectScreen( $lang , $this->settings['base_url'] . "showtopic={$this->topic['tid']}&amp;page=" . $this->request['page'], $this->topic['title_seo'], 'showtopic' );//-IBR-баг у IPS, в новых версиях нужен page, а не st
	}
	...
	public function topicDeletePollVotes()
	{
		...
		if ( ! $voter )
		{
			$this->registry->output->redirectScreen( $this->lang->words['poll_vote_deleted'], $this->settings['base_url'] . "showtopic={$this->topic['tid']}&amp;page=" . $this->request['page'], $this->topic['title_seo'], 'showtopic' );//-IBR-в 3.4 используется page вместо st, а тут забыли
		}
		...
		/* done */
		$this->registry->output->redirectScreen( $this->lang->words['poll_vote_deleted'], $this->settings['base_url'] . "showtopic={$this->topic['tid']}&amp;page=" . $this->request['page'], $this->topic['title_seo'], 'showtopic' );//-IBR- и тут тоже перепутали st и page из-за этого после голосоване возвращает на первую страницу темы
	}
}
