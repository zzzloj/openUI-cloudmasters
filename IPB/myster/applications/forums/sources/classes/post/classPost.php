...
class classPost
{
		...
		if ( $topicTitle )
		{
			...
			if( $this->settings['etfilter_shout'] )
			{
				if( function_exists('mb_convert_case') )
				{
					...
					{
						$this->_topicTitle = ucwords( IPSText::mbstrtolower($this->_topicTitle) );//-IBR- мультибайт
					}
				}
				else
				{
	...
	public function globalSetUp()
	{
		...
		{
			$date						= strtotime( IPSText::monthNameRu2En( $this->request['open_time_date'] ) );//-IBR-переводим месяц на английский (в форме он может быть русским)
		...
		{
			$date						= strtotime( IPSText::monthNameRu2En( $this->request['close_time_date'] ) );//-IBR- в передаваемой формуе месяц мог быть задан по-русски
...
EOF;
	...
	protected function _checkMultiQuote( $postContent )
	{
		...
		else
		{
			...
			//-----------------------------------------
			//-IBR-vulnurability suspection
			$this->request['qpid'] = preg_replace( '/[^,\d]/', "", trim($this->request['qpid']) );//-IBR-возможная уязвимость. qpid должен содержать только цифры
			$this->request['parent_id'] = $this->request['qpid'];
		}
		//cleaning qpid after copy
