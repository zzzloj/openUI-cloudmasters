...
class admin_forums_forums_forums extends ipsCommand
{
	...
	public function forumSave( $type='new' )
	{
		...
		{
			$this->registry->output->global_message = sprintf( $this->lang->words['for_entertitle'], IPSText::mbstrtolower( $forum_cat_lang ) );
			$this->forumForm( $type );
			return;
		}
		...
		if ( $type == 'new' )
		{
			...
			$this->registry->adminFunctions->saveAdminLog( $forum_cat_lang . " '" . $this->request['name'] . "'" . IPSText::mbstrtolower( $this->lang->words['for__created'] ) );
		}
		else
		{
			...
			$this->registry->adminFunctions->saveAdminLog( $forum_cat_lang." '" . $this->request['name'] . "' " . IPSText::mbstrtolower ( $this->lang->words['for__edited'] ) );
		}
