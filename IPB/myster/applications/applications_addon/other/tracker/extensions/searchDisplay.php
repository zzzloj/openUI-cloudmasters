...
class trackerSearchDisplay implements iSearchDisplay
{
	/**
	...
	public function formatContent( $search_row, $isVnc=false )
	{
		...
		$search_row['last_poster'] = $search_row['last_poster_id'] ? IPSMember::makeProfileLink( $search_row['last_poster_name'], $search_row['last_poster_id'], $search_row['seo_last_name'] ) : ipsRegistry::$settings['guest_name_pre'] . $search_row['last_poster_name'] . ipsRegistry::$settings['guest_name_suf'];
		$search_row['starter']     = $search_row['starter_id']     ? IPSMember::makeProfileLink( $search_row['starter_name'], $search_row['starter_id'], $search_row['seo_first_name'] ) : $this->settings['guest_name_pre'] . $search_row['starter_name'] . $this->settings['guest_name_suf'];
	...
	public function buildFilterSQL( $data )
	{
		...
		if( isset( $data ) && is_array( $data ) && count( $data ) )
		{
			foreach( $data as $field => $_data )
			{
				...
				{
					$return[] = array( 'column' => 'ti.posts', 'operator' => '>=', 'value' => intval( $data['pCount'] ) );
				}
			}
	...
	public function modifySearchQuery( $query, $count_only=false )
	{
		...
		{
			$query->SetFilter( 'private', array( 0 ) );

			if( isset( ipsRegistry::$request['search_app_filters']['tracker']['projects'] ) && is_array( ipsRegistry::$request['search_app_filters']['tracker']['projects'] ) && count( ipsRegistry::$request['search_app_filters']['tracker']['projects'] ) )
			{				
				$forum_ids	= array();
