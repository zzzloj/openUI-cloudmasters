<?php
/**
 * @file		plugin_ccscomments.php 	Moderator control panel plugin: show CCS comments pending approval
 *~TERABYTE_DOC_READY~
...
/**
 *
 * @class		plugin_ccs_ccscomments
 * @brief		Moderator control panel plugin: show CCS comments pending approval
 */
class plugin_ccs_ccscomments
{
	/**
	...
	public function executePlugin( $permissions )
	{
		...
		$this->DB->build( array(
								...
								'where'		=> "c.comment_approved=0 AND c.comment_database_id IN(" . implode( ',', $this->databases ) . ")",
								'add_join'	=> array(
													array(
															'select'	=> 'm.*',
		...
		return $this->registry->getClass('output')->getTemplate('ccs_global')->unapprovedComments( $results );
	}
	...
	protected function _getDatabases()
	{
		...
		{
			$this->DB->build( array( 'select' => '*', 'from' => 'ccs_database_moderators', 'where' => 'moderator_approve_comment=1' ) );
