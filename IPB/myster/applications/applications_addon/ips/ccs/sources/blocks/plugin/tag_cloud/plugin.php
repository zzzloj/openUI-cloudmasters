...
class plugin_tag_cloud implements pluginBlockInterface
{
	/**#@+
	...
	public function getTags()
	{
		return array(
					...
					$this->lang->words['block_plugin_tagcloud']	=> array(
																		array( '&#36;output', $this->lang->words['block_plugin_tagcloudd'] ),
																		),
					);
	}
	...
	public function returnPluginInfo()
	{
		return array(
					'key'			=> 'tag_cloud',
					'name'			=> $this->lang->words['plugin_name__tag_cloud'],
					'description'	=> $this->lang->words['plugin_description__tag_cloud'],
					'hasConfig'		=> false,
					'templateBit'	=> 'block__tag_cloud',
					);
	}
	...
	public function executePlugin( $block )
	{
		...
		$cloud->setSkinGroup('ccs_global');
		$cloud->setSkinTemplate('tagCloudPlugin');
		//$cloud->setApp( 'ccs' );	This does work, but restricts the tags to only the current app
		
		$output	= $cloud->render( $cloud->getCloudData( array( 'limit' => 50 ) ) );
