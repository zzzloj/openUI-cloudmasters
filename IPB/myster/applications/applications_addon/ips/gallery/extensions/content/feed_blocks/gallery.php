	...
	public function executeFeed( $block, $previewMode=false )
	{
		...
		switch( $config['content'] )
		{
			case 'images':
				...
				{
					$this->DB->build( array( 'select' => 'perm_type_id as category_id', 'from' => 'permission_index', 'where' => "app='gallery' AND perm_type='categories' AND " . $this->DB->buildRegexp( "perm_view", $this->member->perm_id_array ) ) );
					$this->DB->execute();
						
					while( $r = $this->DB->fetch() )
					{
						$_categories[]	= $r['category_id'];
					}
						
					$where[]	= "i.image_category_id IN(" . ( count($_categories) ? implode( ',', $_categories ) : 0 ) . ")";
				}
				...
				{
					/* @link http://community.invisionpower.com/resources/bugs.html/_/ip-gallery/content-block-not-taking-privacy-settings-into-account-r42091 */
					$where[]	= "i.image_privacy IN (0,1)";
				}
		...
		switch( $config['content'] )
		{
			case 'images':
				...
				while( $r = $this->DB->fetch($outer) )
				{
					...
					$r['id']		= $r['imgid'];
					$r['thumbnail']	= $this->registry->getClass('gallery')->helper('image')->makeImageLink( $r, array( 'type' => 'thumb', 'link-type' => 'page', 'link-thumbClass' => 'galimageview' ) );
			...
			case 'albums':
				...
				while( $r = $this->DB->fetch($outer) )
				{
					...
					$r['id']		= $r['imgid'];
					$r['thumbnail']	= $this->registry->getClass('gallery')->helper('image')->makeImageLink( $r, array( 'type' => 'thumb', 'link-type' => 'container', 'link-thumbClass' => 'galimageview' ) );
