...
class downloads_plugin
{
	...
	public function reportForm( $com_dat )
	{
		...
		$title = $this->DB->buildAndFetch( array( 'select' => 'file_name, file_name_furl', 'from' => 'downloads_files', 'where' => 'file_id=' . intval($file) ) );
		...
		$this->registry->output->addNavigation( IPSLib::getAppTitle( 'downloads' ), "app=downloads", 'downloads', 'false' );
		...
		$url = $this->registry->output->buildSEOUrl( "app=downloads&amp;showfile=" . $ex_form_data['file'], 'public', $title['file_name_furl'], 'idmshowfile' );
	...
	public function processReport( $com_dat )
	{
		...
		$comment	= intval($this->request['comment']);
		$st			= intval($this->request['st']);
		$url		= 'app=downloads&showfile=' . $file . '&st=' . $st . '#comment_' . $comment;
		$file		= $this->DB->buildAndFetch( array( 'select' => 'file_name_furl', 'from' => 'downloads_files', 'where' => 'file_id=' . $file ) );
		...
		$uid			= md5(  'idm___' . $comment . '_' . $com_dat['com_id'] );
		$status			= array();
