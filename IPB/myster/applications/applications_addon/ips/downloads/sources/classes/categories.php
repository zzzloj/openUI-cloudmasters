...
class class_categories
{
	...
	public function normalInit()
	{
		if ( ! $this->init )
		{
			...
			if ( ! is_array( $this->cache->getCache( 'idm_cats' ) ) )
			{
				$this->init_failed = 1;
				$this->fullInit( true );
			}
			...
			{
				foreach( $this->cache->getCache( 'idm_cats' ) as $parentid => $cid )
				{
	...
	public function fullInit( $skipMerge=false )
	{
		if ( ! $this->init )
		{
			...
			$this->cache->updateCacheWithoutSaving( 'idm_cats', $cache );
			...
			if( $this->init_failed )
			{
				$this->cache->setCache( 'idm_cats', $cache, array( 'array' => 1 ) );
				$this->init_failed = 0;
			}
		}
			...
			if( count($children) )
			{
				foreach( $children as $_child )
				{
					if( in_array( $_child, $this->member_access['show'] ) )
					{
						...
						$cat['cfileinfo']['total_downloads']	+= $_childCat['cfileinfo']['total_downloads'];
	...
	public function buildModerators( $try_cache=true )
	{
		...
		if( $try_cache )
		{
			if( $this->cache->exists('idm_mods') )
			{
				$use	= $this->cache->getCache('idm_mods');
				$loaded	= true;
			}
		}
		...
		if( !$loaded )
		{
			$this->DB->build( array( 'select' => '*', 'from' => 'downloads_mods' ) );
			$this->DB->execute();
	...
	public function _rebuildCategoryFileinfo( $catid='all' )
	{
		 ...
		 * @link	http://community.invisionpower.com/tracker/issue-25418-members-can-view-file-from-child-category-in-latest-file-they-have-no-permission-to-view/
		 */
		...
		$stats = $this->DB->buildAndFetch( array( 'select' => 'COUNT(file_id) as files, SUM(file_downloads) as downloads, SUM(file_views) as views',
		...
		/* Data Hook Location */
		IPSLib::doDataHooks( $stats_array, 'downloadUpdateCategoryInfo' );
		
 		$this->DB->update( 'downloads_categories', array( 'cfileinfo' => serialize($stats_array) ), 'cid=' . $catid );
	...
	public function rebuildStatsCache()
	{
		...
		$dlcnt = $this->DB->buildAndFetch( array( 'select' => 'SUM(file_downloads) as dls',
												  'from'   => 'downloads_files',
												  'where'  => 'file_open=1'
										  )		 );
												
		$cache['total_downloads'] = $dlcnt['dls'];
		...
		/* Data Hook Location */
		IPSLib::doDataHooks( $cache, 'downloadRebuildStatsCache' );
		
		$this->cache->setCache( 'idm_stats', $cache, array( 'array' => 1, 'donow' => 0 ) );
		return TRUE;
	}
	...
	public function rebuildModCache()
	{
		...
		$this->cache->setCache( 'idm_mods', $cache, array( 'array' => 1, 'donow' => 1 ) );		
	}
	 ...
	 */
	public function getNav( $catid, $querybit='app=downloads&amp;showcat=', $acp=false )
	{
		if( $acp )
		{
	...
	public function setMemberPermissions( $memid="" )
	{
		...
		$classToLoad	= IPSLib::loadLibrary( IPSLib::getAppDir( 'downloads' ) . '/extensions/coreExtensions.php', 'downloadsPermMappingCat', 'downloads' );
