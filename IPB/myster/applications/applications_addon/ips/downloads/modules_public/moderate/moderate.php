...
class public_downloads_moderate_moderate extends ipsCommand
{
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		switch( $this->request['do'] )
		{
			...
			default:
				$this->registry->output->silentRedirect( $this->settings['base_url'] . 'app=core&module=modcp&fromapp=downloads&tab=brokenfiles' );
				break;
		}
	...
	protected function _updateFileAuthor()
	{
		...
		if( $member['member_id'] )
		{
			...
			$this->DB->update( 'core_tags', array( 'tag_member_id' => $member['member_id'] ), "tag_meta_app='downloads' AND tag_meta_area='files' AND tag_meta_id={$id}" );
			...
			//-----------------------------------------
			// Update topic too
			// @link	http://community.invisionpower.com/tracker/issue-26436-topic-author-is-not-the-same-of-file-when-change-author/
			// @link	http://community.invisionpower.com/tracker/issue-31954-author-not-updated-in-first-post-of-topic-when-changing-the-file-submitter
			//-----------------------------------------
	...
	protected function _doToggleFile( )
	{
		...
		{
			$this->registry->output->redirectScreen( $text, $this->settings['base_url'] . "app=core&amp;module=modcp&amp;fromapp=downloads&amp;tab=unapprovedfiles" );
		}		
	}
	...
	protected function _doDelete( )
	{
		...
		{
			$this->registry->output->redirectScreen( $this->lang->words['moderate_filedeleted'], $this->settings['base_url'] . "app=core&amp;module=modcp&amp;fromapp=downloads&amp;tab=" . ( $this->request['type'] == 'broke' ? 'brokenfiles' : 'unapprovedfiles' ) );
		}
	}
	...
	protected function _doModAction( )
	{
		...
		while( $r = $this->DB->fetch() )
		{
			...
			$canbroke	= $this->registry->getClass('idmFunctions')->checkPerms( $r, 'modcanbrok' );
	 		...
	 		switch( $this->request['type'] )
	 		{
		 		...
		 		default:
					$this->registry->output->redirectScreen( $this->lang->words['modact_message_huh'], $this->settings['base_url'] . "app=core&amp;module=modcp&amp;fromapp=downloads&amp;tab=unapprovedfiles" );
			 	break;
		 	}
	 	}
 		...
 		switch( $this->request['type'] )
 		{
	 		case 'app':
	 			...
	 			{
	 				$this->registry->output->redirectScreen( $this->lang->words['modact_message_huh'], $this->settings['base_url'] . "app=core&amp;module=modcp&amp;fromapp=downloads&amp;tab={$returnAction}" );
	 			}
	 		break;
		...
		$this->registry->output->redirectScreen( $this->message, $this->settings['base_url'] . "app=core&amp;module=modcp&amp;fromapp=downloads&amp;tab={$returnAction}" );
	}	
	...
	protected function _reportBroken()
    {
		...
		if ( !$this->request['do_broken'] )
		{
			...
			/* Output */
			$this->output = $this->registry->getClass('output')->getTemplate('downloads_submit')->submitBrokenFile( $file, $editor->show( 'Post' ) );
			return;
		}
		...
		if( is_array($moderators) AND count($moderators) )
		{
			...
			$_modPanel	= $this->registry->output->buildSEOUrl( 'app=core&module=modcp&fromapp=downloads&tab=brokenfiles', 'public' );
