<?php
/**
 * @file		categories.php 	IP.Downloads category management
 *~TERABYTE_DOC_READY~
 * $Copyright: (c) 2001 - 2011 Invision Power Services, Inc.$
 * $License: http://www.invisionpower.com/company/standards.php#license$
 * $Author: bfarber $
 * @since		1st April 2004
 * $LastChangedDate: 2012-03-02 14:39:28 -0500 (Fri, 02 Mar 2012) $
 * @version		v2.5.4
 * $Revision: 10388 $
 */
...
/**
 *
 * @class		admin_downloads_categories_categories
 * @brief		IP.Downloads category management
 */
class admin_downloads_categories_categories extends ipsCommand
{
	...
	protected function _mainSave( $type='new' )
	{
		...
		IPSLib::loadInterface( 'admin/download_category_form.php' );
		
		$_formPlugins = array();
		
		foreach( IPSLib::getEnabledApplications() as $app_dir => $app_data )
		{
			if ( is_file( IPSLib::getAppDir( $app_dir ) . '/extensions/admin/download_category_form.php' ) )
			{
				$_class  = IPSLib::loadLibrary( IPSLib::getAppDir( $app_dir ) . '/extensions/admin/download_category_form.php', 'admin_download_category_form__'.$app_dir, $app_dir );
		...
		$permissions->savePermMatrix( $this->request['perms'], $type == 'new' ? $category_id : $this->request['c'], 'cat', 'downloads' );
	...
	protected function _mainForm( $type='edit' )
	{
		...
		$classToLoad	= IPSLib::loadLibrary( IPS_ROOT_PATH . 'sources/classes/class_public_permissions.php', 'classPublicPermissions' );
		$permissions	= new $classToLoad( ipsRegistry::instance() );
		$form['permissions']	= $permissions->adminPermMatrix( 'cat', $this->registry->getClass('categories')->cat_lookup[ $cat['cid'] ], 'downloads', '', false );
