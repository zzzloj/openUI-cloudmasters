	...
	*/	
	public function parseEntry( $entry, $no_readupdate=false, $noParse=array(), $blog=null )
	{
		...
		if ( ! $this->_futureUpdated AND ! $noParse['futureApprove'] AND ( $entry['entry_future_date'] AND $entry['entry_date'] <= time() ) )
		{
			...
			/* May as well do all blogs */ // No let's not as then notifications won't be sent.
		...
		if( in_array( $blog['blog_view_level'], array( 'private','privateclub' ) ) && $this->memberData['member_id'] != $blog['member_id'] &&
		...
		if( $album_id > 0 && IPSLib::appIsInstalled('gallery') )
		{
			...
			else
			{
				...
				$this->DB->build( array(
												'select'	=> '*',
												'from'	    => 'gallery_images',
												'where'  	=> "image_member_id = {$this->blog['member_id']} AND image_album_id={$album_id}",
												'order'	    => 'image_id DESC',
												'limit'  	=> array( 0, $imagelimit )
				)	);
			}
			...
			{				
				$image_rows[] = $this->images->makeImageLink( $image, array( 'type' => 'thumb', 'link-type' => 'page' ) );
			}
