...
class postFunctions extends blogPost
{
	...
	public function __construct( ipsRegistry $registry )
	{
		...
		if ( $this->_blogThis && ! $this->registry->isClassLoaded('blogThis') )
		{
	...
	public function addNewEntry()
	{
		...
		$this->registry->tags->add( $_POST['ipsTags'], array(
			'meta_id'			=> $entry_id,
			'meta_parent_id'	=> $this->blog_id,
			'member_id'			=> $this->memberData['member_id'],
			'meta_visible'		=> ( $this->entry['entry_status'] == 'published' ) ? 1 : 0, 
		...
		if( $this->entry['entry_status'] == 'published' )
		{
			IPSMember::sendSocialShares( array( 'title' => $this->entry['entry_name'], 'url'=> $this->registry->output->buildSEOUrl( "app=blog&module=display&section=blog&blogid={$this->entry['blog_id']}&showentry={$this->entry['entry_id']}", 'public', $this->entry['entry_name_seo'], 'showentry' ) ) );
		}
