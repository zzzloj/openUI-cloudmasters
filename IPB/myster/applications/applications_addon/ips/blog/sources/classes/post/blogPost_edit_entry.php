 ...
 * Last Updated: $LastChangedDate: 2012-12-19 14:14:10 -0500 (Wed, 19 Dec 2012) $
 * </pre>
 *
...
class postFunctions extends blogPost
{
	...
	public function completeEdit()
	{
		...
		$where		= array( 'meta_parent_id'	=> $this->blog_id,
		...
		if ( $this->registry->tags->can( 'add', $where ) AND $this->settings['tags_enabled'] AND ( !empty( $_POST['ipsTags'] ) OR $this->settings['tags_min'] ) )
		{
			...
			{
				$this->output .= "<br />".$this->registry->output->getTemplate('post')->errors( $this->registry->tags->getFormattedError() );
			...
			if( $this->org_entry['entry_poll_state'] )
			{
				if( is_array( $this->poll_questions ) AND count( $this->poll_questions ) )
				{
					if( ! ( $this->memberData['g_is_supmod'] or $this->memberData['_blogmod']['moderate_can_edit_entry'] ) )
					{
						...
						$oldchoices = IPSLib::safeUnserialize( $row['choices'] );
		...
		if ( isset($_POST['ipsTags']) )
		{
			...
			$tagreturn = $this->registry->tags->replace( $_POST['ipsTags'], array(
		...
		/* You likey? */
		if( $this->request['follow'] )
		{
			$_like->add( $this->org_entry['entry_id'], $this->memberData['member_id'], array( 'like_notify_do' => 1, 'like_notify_freq' => 'immediate' ) );
		}
		else
		{
			$_like->remove( $this->org_entry['entry_id'], $this->memberData['member_id'] );
		}
	...
	public function showForm()
	{
		...
		if ( $this->org_entry['entry_author_id'] )
		{
			require_once( IPS_ROOT_PATH . 'sources/classes/like/composite.php' );/*noLibHook*/
			$_like	= classes_like::bootstrap( 'blog', 'entries' );
			$following	= $_like->isLiked( $this->org_entry['entry_id'], $this->org_entry['entry_author_id'] );
		}
