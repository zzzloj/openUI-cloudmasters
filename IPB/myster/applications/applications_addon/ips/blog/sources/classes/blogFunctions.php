 ...
 * Last Updated: $LastChangedDate: 2013-05-17 19:05:15 -0400 (Fri, 17 May 2013) $
 * </pre>
 *
...
class blogFunctions
{
	...
	*/
	...
	public function loadBlog( $blog_id="", $member=null )
	{
		...
		$this->DB->build( array(
									...
									'add_join'	=> array( 
															...
															$this->registry->classItemMarking->getSqlJoin( array( 'item_app_key_1' => 'b.blog_id' ) )
														)	) 
						);
		...
		while( $blog = $this->DB->fetch( $i ) )
		{
				...
				$blog = $this->registry->classItemMarking->setFromSqlJoin( $blog );
	...
	public function setActiveBlog( $blog, $silent_error=0 )
	{
		...
		if ( $this->ownsBlog( $blog ) or $this->memberData['_blogmod']['moderate_can_view_private'] or $this->memberData['g_is_supmod'] )
		{
			$canView = TRUE;
		}
	...
	public function rebuildMyBlogsCache( $member=null, $noBlindSave=false )
	{
		...
		$this->DB->build( array( 'select' => 'blog_id',
								 'from'   => 'blog_blogs',
								 'where'  => 'blog_owner_only=0 AND ( blog_authorized_users IS NOT NULL AND FIND_IN_SET('  . $memberData['member_id'] .  ', blog_authorized_users) )' ) );
	...
	{
		$queuedComments = $this->DB->buildAndFetch( array( 
													'select' => 'COUNT(*) as entry_queued_comments',
													'from'	 => 'blog_comments',
													'where'	 => "entry_id={$entry_id} AND comment_approved != 1"
										)	);
		
		$approvedComments = $this->DB->buildAndFetch( array( 
													'select' => 'COUNT(*) as entry_approved_comments',
													'from'	 => 'blog_comments',
													'where'	 => "entry_id={$entry_id} AND comment_approved = 1"
										)	);
		...
		$update['entry_num_comments']       = intval($approvedComments['entry_approved_comments']);
		$update['entry_queued_comments']    = intval($queuedComments['entry_queued_comments']);
		
		if ( $approvedComments['entry_approved_comments'] > 0 )
		{
	...
	public function changeEntryApproval( $ids, $approved=1, $member=null )
	{
		...
		$update = array( 'entry_status' => $status );
		if ( $approved )
		{
			$update['entry_date'] = time();
		}
		$this->DB->update( 'blog_entries', $update, "entry_id IN (" . implode( ",", $finalIds ) . ")" );
	...
	{
		static $cachedAuthorizedUsers = array();
		...
		{
			if( !isset($cachedAuthorizedUsers[ $memberData['member_id'] ]) )
			{
				$cachedAuthorizedUsers[ $memberData['member_id'] ]	= array();

				$this->DB->build( array( 
										'select' => 'blog_id', 
										'from'	 => 'blog_blogs', 
										'where'	 => "FIND_IN_SET("  . $memberData['member_id'] .  ", blog_authorized_users)" 
									)	);
				$this->DB->execute();
				
				while( $row = $this->DB->fetch() )
				{
					$cachedAuthorizedUsers[ $memberData['member_id'] ][] = $row['perm_type_id'];
				}
			}

			$blog_perms['g_blog_authed_blogs']	= $cachedAuthorizedUsers[ $memberData['member_id'] ];
		}
	...
	public function getEntryExcerpt( $entry )
	{
		...
		elseif( isset($entry['entry']) )
		{
			...
			{
				$brDone = true;
				$_entry = IPSText::truncate( strip_tags( IPSText::stripAttachTag( IPSText::getTextClass('bbcode')->stripAllTags( nl2br( $_entry ) ) ), '<br>' ), $this->settings['blog_entry_short'] );
			}
	...
	*/
	...
	public function sendBlogLikeNotifications($blog, $entry)
	{
		...
		$replace = array(
							...
							'URL'		=> $this->registry->output->buildSEOUrl( 'app=blog&blogid=' . $blog['blog_id'] . '&showentry=' . $entry['entry_id'], 'publicNoSession', $entry['entry_name_seo'], 'showentry' ),
							'BLOG'		=> $blog['blog_name'],
						);
