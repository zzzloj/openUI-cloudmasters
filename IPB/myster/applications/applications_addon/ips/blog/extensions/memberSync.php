 ...
 * IP.Board v2.6.3
 * Blog Extensions
 * Last Updated: $Date: 2012-10-24 11:54:06 -0400 (Wed, 24 Oct 2012) $
 * </pre>
 *
...
class blogMemberSync
{
	...
	public function onSetAsSpammer( $member )
	{
		if ( $member['member_id'] )
		{
			...
			if ( count( $eids ) )
			{			
				$this->DB->build( array( 'select' => 'DISTINCT(blog_id)',
						'from'   => 'blog_entries',
						'where'  => 'entry_id IN(' . implode( ',', $eids ) . ')' ) );
				$outer3 = $this->DB->execute();
					
				while( $row = $this->DB->fetch( $outer3 ) )
				{
					$blogs[$row['blog_id']] = $row['blog_id'];
				}
			...
			if ( count($blogs) )
			{
				...
				{
					$this->DB->update( 'blog_cblock_cache', array( 'cbcache_refresh' => 1 ), "blog_id IN(" . implode( ',', $eids ) . ")" . " AND cbcache_key in('lastcomments')", true );
				}
					
				$this->DB->update( 'blog_lastinfo', array( 'blog_last_comment_20' => "" ), "blog_id IN(" . implode( ',', array_keys( $blogs ) ) . ")", true );
