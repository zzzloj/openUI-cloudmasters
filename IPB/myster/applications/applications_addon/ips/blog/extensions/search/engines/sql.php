 ...
 * IP.Board v2.6.3
 * Basic Blog Search
 * Last Updated: $Date: 2013-05-22 19:53:17 -0400 (Wed, 22 May 2013) $
 * </pre>
 *
...
class search_engine_blog extends search_engine
{
	...
	public function _commentsSearch()
	{
		...
		$this->DB->build( array(
								'select'	=> 'c.comment_id, c.comment_date, c.member_id',
	...
	{	
		...
		if ( $search_tags )
		{
			...
			$tags = classes_tags_bootstrap::run( 'blog', 'entries' )->search( $search_tags, array('meta_app' => 'blog', 'meta_area' => 'entries') );
		...
		if ( !$this->memberData['g_is_supmod'] )
		{
			...
			if ( ! $_member['_blogmod']['moderate_can_view_private'] )
			{
				...
				$where_clause[] = '(bl.blog_authorized_users ' . $this->DB->buildIsNull() . " OR bl.blog_authorized_users='' OR b.entry_author_id=" . $this->memberData['member_id'] . " OR FIND_IN_SET("  . $this->memberData['member_id'] .  ", bl.blog_authorized_users) )";
			}
		}
	...
	public function viewNewContent()
	{
		...
		else
		{
			...
			$entryIds	= $this->registry->getClass('classItemMarking')->fetchReadIds( array(), 'blog', true );
			...
			if ( count( $entryIds ) )
			{
				$this->whereConditions['AND'][]	= "b.entry_id NOT IN (" . implode( ",", $entryIds ) . ')';
			}
		}
	...
	public function remapColumn( $column )
	{
		...
		{
			$column	= ( $column == 'member_id' ) ? 'c.member_id' : $column;
		}
