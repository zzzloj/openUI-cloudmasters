	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		if( !intval($this->request['blogid']) )
		{
			...
			if ( $mid )
			{
				...
				$blogs = $this->blogFunctions->fetchMyBlogs( $member );
				...
				if ( is_array( $blogs ) && count( $blogs ) )
				{
					...
					if ( !empty( $greditor ) )
					{
						$this->registry->output->silentRedirect( $this->blogFunctions->getBlogUrl( $greditor[0]['blog_id'], $greditor[0]['blog_seo_name'] ) );
					}
		...
		if( $this->blog['blog_type'] == "external" )
		{
			...
			{
				$this->registry->output->showError( 'blog_not_enabled', 10677.1, null, null, 404 );
			}
		}
		...
		if ( isset( $this->request['cat'] ) AND ( $this->request['cat'] != '' OR $this->request['cat'] == 0 ) )
		{
			...
			$extra .= ' AND FIND_IN_SET(' . intval( $this->request['cat'] )  . ', entry_category )';
			
			$this->page_title = $this->blog['_categories'][ $this->request['cat'] ]['category_title'] . ' - ' . $this->blog['blog_name'] . ' - ' . $this->settings['board_name'];
