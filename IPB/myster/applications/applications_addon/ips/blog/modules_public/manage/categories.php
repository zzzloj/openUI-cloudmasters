...
{		
	...
	protected function _editCategory()
	{
		if ( $this->request['form_hash'] != $this->member->form_hash )
		{
			$this->registry->output->showError( 'blog_no_permission', 20600003, null, null, 403 );
		}
		
		$category_title = IPSText::truncate( trim( $this->request['title'] ), 32 );
		$category 		= $this->DB->buildAndFetch( array( 'select' => '*',
 												 'from'	  => 'blog_categories',
 												 'where'  => 'category_id=' . intval( $this->request['catid'] ) ) ); 
		...
		if ( $this->blogFunctions->ownsBlog( $blog_id ) )
		{
			if ( $category_title )
			{
				$this->DB->update( 'blog_categories', array( 'category_title'     => $category_title, 'category_title_seo' => IPSText::makeSeoTitle( $category_title ) ), 'category_id=' . $category['category_id'] );
