	...
	public function _completeRegistrationSave()
	{
		...
		else
		{
			IPSMember::save( $mid, array( 'members' => array(
																...
																'members_l_username'		=> $tmp_member['members_l_username'] ? $tmp_member['members_l_username'] : $members_display_name,
																'member_group_id'			=> $member_group_id,
																'members_l_display_name'	=> IPSText::mbstrtolower( $members_display_name )
							)			)	);
		}
			...
			if ( $userData['id'] )
			{
				...
				if ( ! count( $form_errors['dname'] ) AND $this->settings['fb_realname'] != 'any' )
				{
					...
					$max = $this->DB->buildAndFetch( array( 'select' => 'MAX(member_id) as max',
													 		'from'   => 'members',
													 		'where'  => "members_l_display_name LIKE '" . $this->DB->addSlashes( IPSText::mbstrtolower( $userData['_displayName'] ) ) . "%'" ) );
	...
	protected function _revalidateComplete()
	{
		...
		if ( $val['lost_pass'] == 1 )
		{
			...
			IPSText::getTextClass('email')->buildMessage( array(
												...
												'THE_LINK'     => $this->registry->getClass('output')->buildSEOUrl("app=core&module=global&section=lostpass&do=sendform&uid=" . $member['member_id'] . "&aid=" . $val['vid'], 'publicNoSession', 'false'),
												'MAN_LINK'     => $this->registry->getClass('output')->buildSEOUrl("app=core&module=global&section=lostpass&do=sendform", 'publicNoSession', 'false'),
												'EMAIL'        => $member['email'],
												'ID'           => $member['member_id'],
												'CODE'         => $val['vid'],
		...
		else if ( $val['new_reg'] == 1 )
		{
			...
			IPSText::getTextClass('email')->buildMessage( array(
												'THE_LINK'     => $this->registry->getClass('output')->buildSEOUrl( "app=core&module=global&section=register&do=auto_validate&uid=" . $member['member_id'] . "&aid=" . $val['vid'], 'publicNoSession', 'false'),
												'NAME'         => $member['members_display_name'],
		...
		else if ( $val['email_chg'] == 1 )
		{
			...
			IPSText::getTextClass('email')->buildMessage( array(
												'NAME'         => $member['members_display_name'],
												'THE_LINK'     => $this->registry->getClass('output')->buildSEOUrl("app=core&module=global&section=register&do=auto_validate&type=newemail&uid=" . $member['member_id'] . "&aid=" . $val['vid'], 'publicNoSession', 'false'),
												'ID'           => $member['member_id'],
	...
	protected function _autoValidate()
 	{
		...
		else if ( $validate['lost_pass'] == 1 )
		{
			...
			IPSText::getTextClass('email')->buildMessage( array(
															'NAME'		=> $member['members_display_name'],
															'THE_LINK'	=> $this->registry->getClass('output')->buildSEOUrl('app=core&module=usercp&tab=core&area=email', 'publicNoSession', 'false'),
															'PASSWORD'	=> $new_pass,
															'LOGIN'		=> $this->registry->getClass('output')->buildSEOUrl('app=core&module=global&section=login', 'publicNoSession', 'false'),
		...
		*/
			...
			if( $data['qa_id'] )
			{
				...
				if( count($answers) )
				{
					foreach( $answers as $answer )
					{
						...
						if( IPSText::mbstrlen($answer) AND IPSText::mbstrtolower($answer) == IPSText::mbstrtolower($this->request['qa_answer']) )
						{
							$pass	= true;
							break;
						}
		...
		if( $coppa != 1 )
		{
			if( ( $this->settings['reg_auth_type'] == 'user' ) or ( $this->settings['reg_auth_type'] == 'admin' ) or ( $this->settings['reg_auth_type'] == 'admin_user' ) )
			{
				...
				{
					/* Send out the email. */
					$message = array(   'THE_LINK'     => $this->registry->getClass('output')->buildSEOUrl("app=core&module=global&section=register&do=auto_validate&uid=" . urlencode( $member['member_id'] ) . "&aid=" . urlencode( $validate_key ), 'publicNoSession', 'false'),
										'NAME'         => $member['members_display_name'],
										'MAN_LINK'     => $this->registry->getClass('output')->buildSEOUrl("app=core&module=global&section=register&do=05", 'publicNoSession', 'false'),
										'EMAIL'        => $member['email'],
										'ID'           => $member['member_id'],
