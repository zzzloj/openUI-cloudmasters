...
class public_core_global_lostpass extends ipsCommand
{
		...
		else
		{
			...
			if( $this->settings['lp_method'] == 'random' )
			{
				...
				$message = array( 'NAME'		=> $member['members_display_name'],
								  'THE_LINK'	=> $this->registry->getClass('output')->buildSEOUrl( 'app=core&module=usercp&tab=core&area=email', 'publicNoSession', 'flase' ),
								  'PASSWORD'	=> $new_pass,
			...
			else
			{
				...
				*/
				...
				$this->registry->output->silentRedirect( $this->registry->getClass('output')->buildSEOUrl( 'app=core&module=global&section=login&do=autologin&frompass=1' ) );
			}
		}
	} 	
	...
	public function lostPasswordEnd()
	{
		...
		/* Check for input and it's in a valid format. */
		$member_name = trim( IPSText::mbstrtolower( $this->request['member_name'] ) );
		...
		else
		{
			...
			$message = array(   'USERNAME'   => $member['name'],
								...
								'THE_LINK'   => $this->registry->getClass('output')->buildSEOUrl( "app=core&module=global&section=lostpass&do=sendform&uid={$member['member_id']}&aid={$validate_key}", 'publicNoSession', 'false' ),
