...
class skin_menus extends output {
	...
	public function jQueryInit() {
		...
		$html .= <<<EOF
			<script>
				deleteMass = "{$this->lang->words['promenu_delete_alert_mass']}";
			</script>
EOF;
...
EOF;
	...
	public function main_container($data = array()) {
		$html = "";
...
EOF;
		...
		$html .= <<<EOF
			...
			<script>
				boo(document).ready(function(){
					boo("#Menus").menusort('main', {
						...
						postUpdateProcess: function(data) {
								cjAjax({
									url: ipb.vars['front_url'].replace(/&amp;/g, '&') + 'app=promenu&module=preview&section=preview&dot=pre&md5check=' + ipb.vars['md5_hash'],
									postType: "POST",
									data: "group={$key}",
									dataType: "html",
									callBack: function(data) {
            							boo("#preview").html(data);
									}
								});
						}
						} ).MenuOpen();
			...
			<div class="section_title">
				...
				<ul class="context_menu" style="margin-top:15px;">
EOF;
		$html .=<<<EOF
					<li>
						...
						boo(document).ready(function(){
							...
							{							
								cjAjax({
									...
									dataType: "html",
									callBack: function(data) {
            							boo("#preview").html(data).promise().done(function(){ boo("#preview").show("slide", {direction: "up"}, 1000); });
									}
								});
							}
							boo("#show_prev").click(function(){
								if(boo(this).data("status") == 1){
									...
									cjAjax({
										url: ipb.vars['base_url'].replace(/&amp;/g, '&') + 'app=promenu&module=ajax&section=ajax&do=status&md5check=' + ipb.vars['md5_hash'],
										data: "group={$key}&status=0"
									});
									cjAjax({
										url: ipb.vars['front_url'].replace(/&amp;/g, '&') + 'app=promenu&module=preview&section=preview&dot=pre&md5check=' + ipb.vars['md5_hash'],
										postType: "POST",
...
EOF;
		...
		if(count($data) && is_array($data)){
			$selectAll =<<<EOF
			{$this->lang->words['promenu_menus_select_all']} <input type="checkbox" name="identity_main" value="1" id="identity_main" class="delo">
EOF;
		}
		$groupName = ucwords($key);
		$html .=<<<EOF
		<form id="itsa" data-key="{$key}">
				<div class="acp-box" style="background:#EAEEF4;">
					<h3>{$groupName} {$this->lang->words['promenu_menu_group']}  <span class="right" style="display:block" id="selectAll">$selectAll</span></h3>
...
EOF;
			...
			$html .=<<<EOF
				<table class="ipsTable double_pad"  id="noMenus" style="display:none;">
					<tr>
						<td>
							{$this->lang->words['promenu_currently_no_menus']}.
						</td>
			...
			<div style="height:50px;background:white;margin-top:10px;">
				<div class="right">
					<input type="submit" class="button primary" value="{$this->lang->words['promenu_clone_group']}" id="massClone">
				</div>
				<div class="right" id="delMove" style="display:none;margin-right:5px;">
					<input type="submit" class="button primary" value="{$this->lang->words['promenu_word_delete']}" id="massDel">
					<input type="submit" class="button primary" value="{$this->lang->words['promenu_word_move']}" id="massMove">
				</div>
			</div>	
		</form>
...
EOF;
			...
			$html .=<<<EOF
				<div class="right" style="text-align:center;margin:5px;padding-top:10px;">
					<input type="checkbox" name="identity_{$c['promenu_menus_id']}" value="1" id="identity_{$c['promenu_menus_id']}" data-id="{$c['promenu_menus_id']}" data-pid="{$c['promenu_menus_parent_id']}" class="delo">
				</div>
...
EOF;
			if (!$c['promenu_menus_parent_id']) {
				$html .=<<<EOF
									<li class='icon copy'> 
										<a href="#" id="singleClone{$c['promenu_menus_id']}" data-id="{$c['promenu_menus_id']}" title="{$this->lang->words['promenu_word_clone']}">{$this->lang->words['promenu_word_clone']}</a>
									</li>
EOF;
			}
			$html .=<<<EOF
...
EOF;
	...
	public function containers($items = array()) {
...
EOF;
		...
		if (count($items)) {
			$html .= "<input type='hidden' name='promenu_menus_id' id='promenu_menus_id' value='" . $items['promenu_menus_id'] . "' >";
		}
...
EOF;
		...
		$html .=<<<EOF
			...
			<div id="erroShow" style="display:none;">
				<ul style="list-style-type:circle;margin-left:25px;">
					<li id="noName" style="display:none;">{$this->lang->words['promenu_no_title_blank']}</li>
					
					<li id="noIMGurl" style="display:none;">{$this->lang->words['promenu_no_title_img_url_blank']}</li>
					
					<li id="noTimgW" style="display:none;">{$this->lang->words['promenu_no_title_img_icon_width_blank']}</li>
					<li id="noTimgWnumeric" style="display:none;">{$this->lang->words['promenu_title_img_icon_width_only_numeric']}</li>
					
					<li id="noTimgH" style="display:none;">{$this->lang->words['promenu_no_title_img_icon_height_blank']}</li>
					<li id="noTimgHnumeric" style="display:none;">{$this->lang->words['promenu_title_img_icon_height_only_numeric']}</li>	
					
					
					<li id="noIMGurli" style="display:none;">{$this->lang->words['promenu_no_icon_url_blank']}</li>
					
					<li id="noTimgWi" style="display:none;">{$this->lang->words['promenu_no_icon_url_blank']}</li>
					<li id="noTimgWnumerici" style="display:none;">{$this->lang->words['promenu_icon_width_only_numeric']}</li>
					
					<li id="noTimgHi" style="display:none;">{$this->lang->words['promenu_no_icon_height_blank']}</li>
					<li id="noTimgHnumerici" style="display:none;">{$this->lang->words['promenu_icon_height_only_numeric']}</li>
					
					<li id="napp" style="display:none;">{$this->lang->words['promenu_menus_blank_napp']}</li>
					<li id="nccs" style="display:none;">{$this->lang->words['promenu_select_content_page']}</li>
					<li id="neasy" style="display:none;">{$this->lang->words['promenu_select_easypage_page']}</li>
					<li id="nblock" style="display:none;">{$this->lang->words['promenu_no_block_code_blank']}</li>
					
					<li id="phpValid" style="display:none;">{$this->lang->words['promenu_php_code_not_valid']}</li>
					
					<li id="nwrapUrl" style="display:none;">{$this->lang->words['promenu_menus_wrap_no_url']}</li>
					
					<li id="nwrapH" style="display:none;">{$this->lang->words['promenu_menus_wrap_non_numberic_height']}</li>
					<li id="nwrapHBlank" style="display:none;">{$this->lang->words['promenu_menus_wrap_blank']}</li>
					<li id="nmegacount" style="display:none;">{$this->lang->words['promenu_number_columns_only_numeric']}</li>
					<li id="nopage" style="display:none;">{$this->lang->words['promenu_nopage']}</li>
				</ul>
			</div>
...
EOF;
		...
		$html .=<<<EOF
			...
			<div class='acp-actionbar'>
				<input type='submit' class='button primary' onclick="return ProValidate()" value="{$this->lang->words['promenu_word_save']}" />
				<input type='submit' class='button primary' onclick="return ProValidate()" value="{$this->lang->words['promenu_save_reload']}" name='return' />
				<input type='button' class='button redbutton' name='cancel' onclick="goBack();" value="{$this->lang->words['promenu_word_cancel']}" />
			</div>		
		</form>
EOF;
		$html .= <<<EOF
		<script type='text/javascript'>
			boo("#tabstrip_add").ipsTabBar({ tabWrap: "#tabstrip_add_content" });
...
EOF;
	...
	protected function tab1($items = array()) {
		...
		<table id='catItem_table' class='ipsTable'>
			<tr class='ipsControlRow' id="proName">
...
EOF;
		...
		$html .=<<<EOF
				<tr class='ipsControlRow'>
					<td class='field_title'><strong class='title'>{$this->lang->words['promenu_menus_more_disable']}</strong></td>
					<td class='field_field'>{$this->registry->output->formYesNo('promenu_menus_more_disable', $items['promenu_menus_more_disable'])}<br />
						<span class='desctext'>
							{$this->lang->words['promenu_menus_more_disable_desc']}.
						</span>
					</td>
				</tr>
EOF;
			...
			$html .=<<<EOF
			<tr class='ipsControlRow' id="proTitleImage">
...
EOF;
		...
		$html .=<<<EOF
			<tr class='ipsControlRow' id="proTitleImagei">
...
EOF;
		if(!intval($items['promenu_menus_forums_id'])){
			$html .=<<<EOF
			<tr class='ipsControlRow'>
	...
	protected function tab3($items = array()) {
		...
		$html .= <<<EOF
				<table id='menuItem_table' class='ipsTable'>
					...
					<tr  id="megaCount" class='ipsControlRow'>
	...
	public function group_settings() {
		...
		<script>
			boo(document).ready(function(){
				...
				var TOC = "{$group['promenu_groups_make_super']}";
				...
				boo("#promenu_groups_make_super_yes").click(function(){
					boo(".noMain").fadeOut();
				});
				
				boo("#promenu_groups_make_super_no").click(function(){
...
EOF;
		...
		$html .=<<<EOF
				<tr class='ipsControlRow mobiles'>
					<td class='field_title'><strong class='title'>{$this->lang->words['promenu_group_make_super']}</strong></td>
					<td class='field_field'>{$this->registry->output->formYesNo('promenu_groups_make_super', $group['promenu_groups_make_super'] ? $group['promenu_groups_make_super'] : 0)}<br />
					<span class='desctext'>{$this->lang->words['promenu_group_make_super_desc']}</span></td>
				</tr>
EOF;
		}
		if($group['promenu_groups_name'] !== "primary"){
		$html .=<<<EOF
				<tr class='ipsControlRow  noMain '>
					<td class='field_title'><strong class='title'>{$this->lang->words['promenu_groups_add_other']}</strong></td>
					<td class='field_field'>{$this->registry->output->formYesNo('promenu_groups_add_other', $group['promenu_groups_add_other'])}<br />
						<span class='desctext'>
							{$this->lang->words['promenu_groups_add_other_desc']}.
						</span>
					</td>
				</tr>
EOF;
...
EOF;
		$html .=<<<EOF
				<tr class='ipsControlRow  noMain mobiles'>
					<td class='field_title'><strong class='title'>{$this->lang->words['promenu_menu_more_enabled']}</strong></td>
					<td class='field_field'>{$this->registry->output->formYesNo('promenu_groups_promore', $group['promenu_groups_promore'])}<br />
						<span class='desctext'>
							{$this->lang->words['promenu_menu_more_enabled_desc']}
						</span>
					</td>
				</tr>
EOF;
...
EOF;
		$html .=<<<EOF
				<tr class='ipsControlRow noMain mobiles'>
					<td class='field_title'><strong class='title'>{$this->lang->words['promenu_group_allow_dock']}</strong></td>
					<td class='field_field'>{$this->registry->output->formYesNo('promenu_groups_allow_docking', $group['promenu_groups_allow_docking'])}<br />
					<span class='desctext'>{$this->lang->words['promenu_group_allow_dock_desc']}</span></td>
				</tr>
EOF;
		$html .=<<<EOF
					<tr class='ipsControlRow noMain '>
...
EOF;
		$html .=<<<EOF
					<tr class='ipsControlRow noMain mobiles'>
						<th class="subhead" style="width: 78%" colspan="2">
							<span class="larger_text">Mobile Device Settings</span>
						</th>
					</tr>
					<tr class='ipsControlRow  noMain mobiles'>
						<td class='field_title'><strong class='title'>Default to on mobile device</strong></td>
						<td class='field_field'>
						<span id="name_title">{$this->registry->output->formDropDown('promenu_groups_default_mobile', array( array(1, "Promenu Pop Up"),array(2 , "Use only ProMore"), array( 3 ,"Mobile Style")),$group['promenu_groups_default_mobile'])}</span>
						<br />
						<span class='desctext'>Which menu style should promenu default to when mobile device is being used?</span></td>
					</tr>
EOF;
		if($group['promenu_groups_name'] !== "header") {
		$html .=<<<EOF
					<tr class='ipsControlRow  noMain mobiles'>
						<td class='field_title'><strong class='title'>Mobile Style Default Positioning</strong></td>
						<td class='field_field'>
						<span id="name_title">{$this->registry->output->formDropDown('promenu_groups_default_position', array( array(1, "Fixed"),array(2 , "Cover")),$group['promenu_groups_default_position'])}</span>
						<br />
						<span class='desctext'>Fixed: pushes main body content down, Cover: slides over main body content.</span></td>
					</tr>
EOF;
	}
		$html .=<<<EOF
					<tr class='ipsControlRow  noMain mobiles'>
						<td class='field_title'><strong class='title'>Mobile Style min width</strong></td>
						<td class='field_field'>
							<span id="name_title">{$this->registry->output->formSimpleInput('promenu_groups_min_width',$group['promenu_groups_min_width'])}</span>
							<br />
							<span class='desctext'>what is the minimum width of window before mobile style kicks in.</span>
						</td>
					</tr>		
EOF;
		$html .=<<<EOF
