...
class public_testimonials_testemunhos_list extends ipsCommand
{
	private $output;
	private $library;
	...
	private function listall()
	{
        ...
        $cid = intval( $this->request['showcat'] );

		$theviews = $this->DB->buildAndFetch( array(  'select'    => '*',
								 					  'from'      => 'testemunhos_cats',
								 					  'where'     => 'c_id='.$cid) );
        
        if( $theviews['t_test'] >= 1 )
        {                                             
		...
		$testemunhosPages = $this->output->generatePagination( array( 'totalItems'        => $totalTestemunhos['found'],
                                                                      ...
                                                                      'baseUrl'           => "app=testimonials&module=testemunhos&section=list&showcat={$cid}"
		) );
        ...
        $count 		= $this->DB->buildAndFetch( array( 'select'   => 'count(*) as total', 'from'  => 'testemunhos_cats' ) );
         
        $this->registry->output->addNavigation( $cat['c_name'], '' );
                                                        
		$this->library->pageOutput .= $this->registry->getClass('output')->getTemplate('testimonials')->listTestemunhosWrapper( $testemunhos_data, $filters, $testemunhosPages, $this->library->getActiveUsers(), $cat, $count );
    }
	...
	public function parseTestemunhoData( $testemunho )
	{
        ...
        if($hasComm['t_comments'])
        {
            ...
            $testemunho['last_author']	    = IPSMember::makeProfileLink( IPSMember::makeNameFormatted( $v4, $v5 ), $v1, $v3 );   				
        ...
        else if(!$hasComm['t_comments'])
        {
            ...
            $testemunho['last_author']	    = IPSMember::makeProfileLink( IPSMember::makeNameFormatted( $v9, $v10 ), $v6, $v8 );   			
