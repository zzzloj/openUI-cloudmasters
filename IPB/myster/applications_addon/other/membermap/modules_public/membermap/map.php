...
/**
 * IP.Board Member Map - Map Section
 *
...
class public_membermap_membermap_map extends ipsCommand
{
    /**
     * Class entry point
     *
    ...
    public function doExecute( ipsRegistry $registry )
    {
        ...
        $this->registry->output->setTitle( IPSLib::getAppTitle('membermap') );

        $friends = (isset($this->request['do']) && $this->request['do'] == 'friends') ? TRUE : FALSE;
        ...
        switch($this->request['action'])
        {
            ...
            break;
            case 'removeMarker':
                $this->doModerateRemoveMarker();
            break;
        }
        $this->registry->output->addNavigation( IPSLib::getAppTitle('membermap'), 'app=membermap', IPSText::makeSeoTitle( IPSLib::getAppTitle('membermap') ), 'app=membermap' );
    ...
    /**
     * Check permissions and delete members location
     */
    public function doDelete()
    {
        ...
        if ( $this->registry->permissions->check( 'delete', $perm ) )
        {
            $this->dB->delete('member_map', 'member_id='. $this->memberData['member_id']);
            $this->registry->output->addContent( $this->registry->getClass('output')->getTemplate('membermap')->mapStatus( $this->lang->words['locationDeleted'] ) );
        }
        else
        {
            $this->registry->getClass('output')->showError( 'noPermissionDelete', 1903, FALSE );
        }
    }
    ...
    public function doAdd()
    {
        ...
        if ( $this->registry->permissions->check( 'add', $perm ) )
        {
            ...
            {
                $this->registry->getClass('output')->showError( 'locationAlreadyAdded', 1904, FALSE );
            }
            ...
            if(isset($this->request['lat']) && isset($this->request['lon']))
            {
                ...
                $this->registry->output->addContent( $this->registry->getClass('output')->getTemplate('membermap')->mapStatus( $this->lang->words['locationAdded'] ) );
            ...
            {
                $this->registry->output->addContent( $this->registry->getClass('output')->getTemplate('membermap')->mapStatus( $this->lang->words['problemAddingLocation'] ) );
            }
        }
        else
        {
            $this->registry->getClass('output')->showError( 'noPermissionAdd', 1903, FALSE );
        }
    }
    ...
    public function doUpdate()
    {
        ...
        if ( $this->registry->permissions->check( 'edit', $perm ) )
        {
            ...
            if($this->dB->getTotalRows() == 0)
            {
                $this->registry->getClass('output')->showError( 'noUpdateExists', 1906, FALSE );
            }
            ...
            if(isset($this->request['lat']) && isset($this->request['lon']))
            {
                ...
                $this->registry->output->addContent( $this->registry->getClass('output')->getTemplate('membermap')->mapStatus( $this->lang->words['locationUpdated'] ) );
            ...
            {
                $this->registry->output->addContent( $this->registry->getClass('output')->getTemplate('membermap')->mapStatus( $this->lang->words['problemUpdating'] ) );
            }
        }
        else
        {
            $this->registry->getClass('output')->showError( 'noPermissionUpdate', 1905, FALSE );
        }
    }

    /**
     * Removes a marker if you are a super moderator.
     */
    public function doModerateRemoveMarker()
    {
        if($this->memberData['g_is_supmod'] == 1)
        {
            $this->dB->delete('member_map', 'member_id='. intval($this->request['do']));
            $this->registry->output->addContent( $this->registry->getClass('output')->getTemplate('membermap')->mapStatus( $this->lang->words['locationDeleted'] ) );
        }
        else
        {
            $this->registry->getClass('output')->showError( 'noPermissionDelete', 19010, FALSE );
        }
    }
     ...
     */
    public function doMap($friends = FALSE)
    {
        $perm = $this->_loadMapPermissions();

        if ( $this->registry->permissions->check( 'view', $perm ) )
        {
            $this->registry->output->addToDocumentHead('javascript', $this->settings['board_url'] .'/public/js/ips.membermap.js');
            ...
            if(isset($this->settings['membermapMapService']) && $this->settings['membermapMapService'])
            {
                if(file_exists(IPSLib::getAppDir( 'membermap' ) . '/sources/classes/'.$this->settings['membermapMapService'].'.php'))
                {
                    ...
                    if(isset($this->settings[$key]) && !empty($this->settings[$key]))
                    {
                        ...
                        $canDelete = $this->registry->permissions->check( 'delete', $perm );

                        $this->registry->output->addContent(
								$this->registry->getClass('output')->getTemplate('membermap')->mapTemplate( $canAdd, $canEdit, $canDelete, $exists, $this->mapFunctions->countMarkers(), $this->mapFunctions->countCMarkers() )
						);

                        $this->mapFunctions->addMapJsToTemplate();
	...
	private function _guestCheck()
	{
		...
		{
			$this->registry->getClass('output')->showError( 'guestsCannotAdd', 1913, FALSE );
		}
	}
}