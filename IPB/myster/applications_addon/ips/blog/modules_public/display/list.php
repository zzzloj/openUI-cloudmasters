 ...
 * $LastChangedDate: 2012-10-24 15:54:01 -0400 (Wed, 24 Oct 2012) $
 * @version		v2.6.3
 * $Revision: 11516 $
 */
...
class public_blog_display_list extends ipsCommand
{
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		$this->request['selectedbids'] = IPSCookie::get('modbids');
	...
	protected function _showAll()
	{
		...
		{
			$query[] = "( ( b.blog_owner_only=1 AND b.member_id="  . intval( $this->memberData['member_id'] ) . " ) OR b.blog_owner_only=0 ) AND ( FIND_IN_SET("  . intval( $this->memberData['member_id'] ) .  ", b.blog_authorized_users) OR b.blog_authorized_users IS NULL )";
		}
		...
		{		
			...
			while( $blog = $this->DB->fetch( $o ) )
			{
				...
				$blog['last_read']	 = $this->registry->classItemMarking->fetchTimeLastMarked( array( 'blogID' => $blog['blog_id'] ), 'blog' );
		...
		/* Set canonical */
		$this->registry->output->addCanonicalTag( ( $this->request['st'] ) ? 'app=blog' . '&st=' . $this->request['st'] : 'app=blog', 'blogs', 'blogs' );		
		
		/* Push out */
		return $this->registry->getClass('output')->getTemplate('blog_list')->blogIndexPage( $links, $featured, $blogs, $extraData, 'all', $all_sorting_options, $pinned );
	}
	...
	protected function _showDash()
	{
		...
		/* Set canonical */
		$this->registry->output->addCanonicalTag( 'app=blog&type=dash', 'blogs', 'blogs' );
				
		/* Push out */
		return $this->registry->getClass('output')->getTemplate('blog_list')->blogIndexPage( '', $featured, $blogs, $extraData, 'dash', $all_sorting_options, $pinned );
	}
		...
		{ 
			...
			while( $row = $this->DB->fetch( $o ) )
			{
				if ( ! $row['blog_disabled'] AND $row['entry_status'] != 'draft' )
				{
					...
					$row = $this->registry->classItemMarking->setFromSqlJoin( $row );
	 ...
	 * @return	@e array
	 */
	protected function _fetchPinned()
	{
		/* Init vars */
		$pinned = array();
		...
		if ( $this->caches['blog_stats']['stats_num_blogs'] )
		{
			...
			if ( is_array( $this->caches['blog_stats']['pinned'] ) AND count( $this->caches['blog_stats']['pinned'] ) )
			{
				$this->DB->build( array( 'select'	=> 'b.*, b.blog_id as blog_id_id',
										 'from'		=> array( 'blog_blogs' => 'b' ),
										 'where'	=> 'b.blog_id IN (' . implode( ',', array_values( $this->caches['blog_stats']['pinned'] ) ) . ') AND b.blog_disabled=0 AND blog_type=\'local\' AND b.blog_view_level=\'public\'',
										 'order'    => 'b.blog_last_edate DESC',
										 'limit'    => array( 0, 20 ),
										 'add_join'	=> array(array( 'select'	=> 'bl.*',
																	 'from'	    => array( 'blog_lastinfo' =>'bl' ),
