	...
	*/
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		$_entry_joins = array( array('select' => 'r.rating_id, r.rating as current_rating',
									 ...
									 'type'	  => 'left' ),
	  						  array( 'select' => 'm.member_id, m.members_display_name as entry_author_name, m.email, m.member_group_id, m.mgroup_others, m.members_seo_name', 
		...
		/* Item Marking Efficiency */
		$entry = $this->registry->classItemMarking->setFromSqlJoin( $entry );
		...
		if ( $this->settings['reputation_enabled'] )
		{
			$entry['has_given_rep']	= $entry['has_given_rep'] ? $entry['has_given_rep'] : 0;
			
			$entry['rep_points'] = $this->registry->getClass('repCache')->getRepPoints( array( 'app' => 'blog', 'type' => 'entry_id', 'type_id' => $entry['entry_id'], 'rep_points' => $entry['rep_points'] ) );
		}
		...
		/* Attachments */
		// @link http://community.invisionpower.com/resources/bugs.html/_/ip-blog/bb-codes-with-attachment-in-their-examples-r42360
		...
		/* Output! */
		$this->registry->output->addToDocumentHead( 'raw', "<link rel='author' href='" . $this->registry->output->buildSEOUrl( 'showuser=' . $entry['entry_author_id'], 'publicNoSession', $entry['members_seo_name'], 'showuser' ) . "' />" );
		
		$this->registry->output->addCanonicalTag( "app=blog&amp;module=display&amp;section=blog&amp;blogid={$this->blog['blog_id']}&amp;showentry={$entry['entry_id']}", $entry['entry_name_seo'], 'showentry' );
		
		$this->registry->output->addMetaTag( 'keywords', preg_replace( '/\s+/', ' ', strip_tags( $entry['entry'] ) ), TRUE );
		$this->registry->output->addMetaTag( 'description', trim( preg_replace( '/\s+/', ' ', strip_tags( $entry['entry']) ) ), FALSE, 155 );
		
		if( $entry['entry_image'] )
		{
			$this->registry->output->addMetaTag( 'og:image', "{$this->settings['upload_url']}/{$entry['entry_image']}", false );
		}
	...
	protected function _checkRedirect()
	{
		if ( $this->request['show'] && $this->request['showentry'] )
		{
			...
			$currentry	= $this->DB->buildAndFetch( array( 'select' => 'entry_id, entry_name_seo, entry_date',
														   'from'   => 'blog_entries',
			...
			if ( $this->request['show'] == 'nextnewest' )
			{
				$newentry	= $this->DB->buildAndFetch( array( 
																'select'	=> 'entry_id, entry_name_seo',
																'from'		=> 'blog_entries',
				...
				{
	       			$this->registry->output->silentRedirect( "{$this->settings['base_url']}app=blog&amp;module=display&amp;section=blog&amp;blogid={$this->blog['blog_id']}&amp;showentry={$newentry['entry_id']}", $newentry['entry_name_seo'], false, 'showentry' );
				}
				else
				{
			...
			if ( $this->blog['blog_num_comments'] )
			{
				if ( $this->request['show'] == 'newcomment' )
				{
					...
					if ( $newcomment['comment_id'] )
					{
						...
						$this->registry->output->silentRedirect( "{$this->settings['base_url']}app=blog&amp;module=display&amp;section=blog&amp;blogid={$this->blog['blog_id']}&amp;showentry={$eid}{$st}&#comment_" . $newcomment['comment_id'], $currentry['entry_name_seo'], false, 'showentry' );
		       		}
		       		else
		       		{
				...
				if ( $this->request['show'] == 'lastcomment' )
				{
					...
					if ( $newcomment['comment_id'] )
					{
		       		...
		       		{
		       			$this->registry->output->silentRedirect( "{$this->settings['base_url']}app=blog&amp;module=display&amp;section=blog&amp;blogid={$this->blog['blog_id']}&amp;showentry={$eid}&#commentsStart", $currentry['entry_name_seo'], false, 'showentry' );
		       		}
				}
				...
				if ( $this->request['show'] == 'comment' )
				{
					...
					if ( $comment_id )
					{
						...
						$st		= ($st > 0) ? "&amp;st=" . $st : "";
		       			$this->registry->output->silentRedirect( "{$this->settings['base_url']}app=blog&amp;module=display&amp;section=blog&amp;blogid={$this->blog['blog_id']}&amp;showentry={$eid}{$st}&#comment_" . $comment_id, $currentry['entry_name_seo'], false, 'showentry' );
		       		}
		       		else
		       		{
		       			$this->registry->output->silentRedirect( "{$this->settings['base_url']}app=blog&amp;module=display&amp;section=blog&amp;blogid={$this->blog['blog_id']}&amp;showentry={$eid}&#commentsStart", $currentry['entry_name_seo'], false, 'showentry' );
