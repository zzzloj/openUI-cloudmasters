 ...
 * RSS import class
 * by Matt Mecham
 * Last Updated: $LastChangedDate: 2013-04-29 20:59:30 -0400 (Mon, 29 Apr 2013) $
 * </pre>
 *
 * @author 		Matt
...
class blogRssImport
{
 			...
 			while( $row = $this->_rssClass->fetch() )
 			{
 				...
 				if ( is_array( $data['rss_cats'] ) AND count( $data['rss_cats'] ) )
 				{
					foreach( $data['rss_cats'] as $cid )
					{
						if ( trim( $cid ) )
						{
							$this->request['catCheckBoxes'][ $cid ] = 1;
						}
					}
	...
	public function _buildEntry( $row, $blog, $memberData, $data )
	{
		...
		$entry = array( 'blog_id'			  => $blog['blog_id'],
						...
						'entry_html_state'	  => intval( $memberData['g_blog_do_html'] ),
						'entry_use_emo'		  => 1,
