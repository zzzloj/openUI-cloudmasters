 ...
 * Last Updated: $Date: 2012-08-09 09:00:38 -0400 (Thu, 09 Aug 2012) $
 * </pre>
 *
...
class blog_hookGateway
{
    ...
    /**
     * Shows the "Blog This" link beneath posts
     *
     * @access	public
     * @return	string		HTML
     */
    public function showBloggedThisLink( $output, $key )
    {
    	/* App switched on? */
    	if ( ! IPSLib::appIsInstalled( 'blog' ) )
    	{
    	...
    	if (is_array( $this->registry->output->getTemplate( 'topic' )->functionData['post'] ) and count( $this->registry->output->getTemplate( 'topic' )->functionData['post'] ))
    	{
    		...
    		foreach ( $this->registry->output->getTemplate( 'topic' )->functionData['post'] as $_post )
    		{
    			...
    			{
    				$post = $_post['post'];
    				$string = $this->registry->getClass('output')->getTemplate('blog_hooks')->topics_blogThisLink( $post );
