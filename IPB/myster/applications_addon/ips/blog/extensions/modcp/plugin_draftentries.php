	...
	public function executePlugin( $permissions )
	{
		...
		if( !$this->memberData['g_is_supmod'] )
		{
			...
			if( !$this->memberData['_blogmod']['moderate_can_view_private'] )
			{
				...
				$_where[]	= '(bl.blog_authorized_users ' . $this->DB->buildIsNull() . " OR bl.blog_authorized_users='' OR b.entry_author_id=" . $this->memberData['member_id'] . " OR FIND_IN_SET("  . $this->memberData['member_id'] .  ", bl.blog_authorized_users) )";
			}
		}
		...
		if ( $count['total'] )
		{
			...
			while( $row = $this->DB->fetch() )
			{
				...
				/* Are we showing private blogs */
				if( in_array( $row['blog_view_level'], array( 'private','privateclub' ) ) && $this->memberData['member_id'] != $row['entry_author_id'] && !in_array($row['blog_id'], $this->memberData['g_blog_authed_blogs']) )
				{
					$row['hide_private'] = 1;
				}
				else
				...
				$row['app']                 = 'blog';
				$row['content']             = ( $row['hide_private'] ) ? $this->lang->words['blog_private_entry'] : $row['entry_short'];
