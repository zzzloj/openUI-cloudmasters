...
class blog_plugin
{
	...
	public function reportForm( $com_dat )
	{
		...
		{
			$url	= $this->registry->output->buildUrl( "app=blog&blogid={$this->entry['blog_id']}&showentry={$ex_form_data['entry_id']}&show=comment&cid={$ex_form_data['comment_id']}", 'public' );
			$this->registry->output->addNavigation( $this->lang->words['report_title'], '' );
		}
		else
		{
			$url = $this->registry->output->buildSEOUrl( "app=blog&blogid={$this->entry['blog_id']}&showentry={$ex_form_data['entry_id']}", 'public', $this->entry['entry_name_seo'], 'showentry' );
	...
	public function processReport( $com_dat )
	{
		...
		if ( $comment_id )
		{
			...
			$message = $this->registry->output->parseIPSTags( $parser->buildQuoteTag( $message, $data['members_display_name'], $data['entry_date'], 0 ) );
