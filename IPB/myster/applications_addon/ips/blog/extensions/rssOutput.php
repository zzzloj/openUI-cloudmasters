...
class rss_output_blog
{
	...
	{		
		...
		if ( !$this->memberData['g_blog_allowview'] )
		{
			if ( empty($this->memberData['member_id']) && !$this->settings['blog_allow_rssguests'] )
			{
				return $this->_returnError( $this->lang->words['rss_not_allowed'] );
			}
		}
		...
		if ( $this->blog_id )
		{
			...
			if ( !$this->blog['blog_allowguests'] || !$this->blog['blog_settings']['allowrss'] || $this->blog['blog_view_level'] != 'public' )
			{
				return $this->_returnError( $this->lang->words['rss_not_allowed'] );
			}
			
			if ( $this->blog['blog_type'] != "local" || $this->blog['blog_disabled'] )
			{
	...
	protected function _recache()
	{
		...
		while ( $entry = $this->DB->fetch( $qid ) )
		{
			...
			$entry['entry'] = IPSText::stripAttachTag( trim($entry['entry']) );
			$entry['entry'] = IPSText::getTextClass('bbcode')->stripAllTags( $entry['entry'], false );
			...
			if( $entry['entry_has_attach'] )
			{
				...
				$_output = $this->class_attach->renderAttachments( $entry['entry'] . "<!--IBF.ATTACHMENT_{$entry['entry_id']}-->", array( $entry['entry_id'] ), 'blog_show' );
	...
	protected function _returnError( $error='' )
	{
		...
		$this->class_rss->addItemToChannel( $channel_id, array( 
														...
														'link'			=> $this->registry->output->buildSEOUrl( "app=blog", 'public', true, 'app=blog' ),
			 										    'description'	=> $error,
			 										    'pubDate'		=> $this->class_rss->formatDate( time() ),
			 										    'guid'			=> $this->registry->output->buildSEOUrl( "app=blog&amp;error=1", 'public', true, 'app=blog' ) ) );
