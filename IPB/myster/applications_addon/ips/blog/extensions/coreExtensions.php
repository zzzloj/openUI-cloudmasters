 ...
 * IP.Board v2.6.3
 * Item Marking
 * Last Updated: $Date: 2012-09-17 12:07:56 -0400 (Mon, 17 Sep 2012) $
 * </pre>
 *
...
class itemMarking__blog
{
	...
	{	
		if ( $this->request['module'] == 'display' && ( empty( $this->request['section'] ) || $this->request['section'] == 'list'))
		{
			return false;
		}
		if ( $this->request['section'] == 'blog')
		{
			return false;
		}
		if ( $this->request['section'] == 'entry')
		{
			return false;
		}
		return true;
	...
	public function parseOnlineEntries( $rows )
	{
		...
		if ( is_array( $rows ) and count( $rows ) )
		{
			...
			if ( count( $blog_ids ) > 0 )
			{
				...
				{
					$query[] = "( ( blog_owner_only=1 AND member_id={$member_id} ) OR blog_owner_only=0 ) AND ( FIND_IN_SET("  . $member_id .  ", blog_authorized_users) OR blog_authorized_users IS NULL )";
				}
			...
			if ( count( $entry_ids ) > 0 )
			{
				...
				ipsRegistry::DB()->build( array( 
												...
												'where'    => "e.entry_status='published' AND 
	                                                          ...
	                                                          ( FIND_IN_SET("  . $member_id .  ", b.blog_authorized_users) OR b.blog_authorized_users IS NULL ) {$allowguests} {$extra}",
