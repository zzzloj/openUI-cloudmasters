...
class public_downloads_display_index extends ipsCommand
{
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		$this->registry->output->setTitle( IPSLib::getAppTitle('downloads') . ' - ' . $this->settings['board_name'] );
	...
	protected function _showPortal()
	{
		...
		{
			$feeds['highrated']	= $idm_api->returnDownloads( 0, 18, true, "file_rating DESC, " . $this->DB->buildLength( "file_votes" ) . " DESC" );
		...
		{			
			...
			$this->DB->build( array(
									...
									'group'		=> 'p.ps_item_id',
									'where'		=> "p.ps_app='downloads' AND p.ps_type='file' AND f.file_open=1 AND f.file_cost != 0 AND f.file_cat IN(" . implode( ',', $categories ) . ") AND f.file_submitted > {$searchRange}", 
			...
			if( count( $paidFiles ) )
			{
				$feeds['topfree']	= $idm_api->returnDownloads( 0, 5, true, "file_downloads DESC", array( "file_cost=0", "file_submitted > {$searchRange}" ) );
				$toppaid	= $idm_api->returnDownloads( 0, 5, true, null, array( "file_id IN(" . implode( ',', $paidFiles ) . ')' ) );
			...
			{
				$feeds['topfiles']	= $idm_api->returnDownloads( 0, 10, true, "file_downloads DESC", array( "file_submitted > {$searchRange}" ) );
		...
		$this->DB->build( array( 'select' => 'file_submitter, SUM(file_downloads) as totalfiles', 'from' => 'downloads_files', 'where' => 'file_open=1 AND file_cat IN (' . implode( ',', $categories ) . ') AND file_submitted > ' . $searchRange, 'order' => 'totalfiles DESC', 'limit' => array( 0, 5 ), 'group' => 'file_submitter' ) );
		$this->DB->execute();
		...
		$this->DB->build( array(
							...
							'where'		=> "l.like_app='downloads' AND l.like_area='files' AND l.like_visible=1 AND f.file_id " . $this->DB->buildIsNull(false) . ' AND f.file_open=1 AND f.file_cat IN (' . implode( ',', $categories ) . ')',
							'limit'		=> array( 0, 18 ),
							'group'		=> 'like_rel_id',
							'add_join'	=> array(
												array(
		...
		while( $r = $this->DB->fetch($outer) )
		{
			if( $r['file_id'] )
			{
				...
				if ( $this->memberData['idm_bypass_paid'] or $r['file_submitter'] == $this->memberData['member_id'] )
				{
					$r['_purchased']	= 'ACTIVE';
				}
				elseif ( ( $r['file_cost'] or $r['file_nexus'] ) and IPSLib::appIsInstalled('nexus') )
				{
					...
					if ( $r['file_cost'] )
					{
						$r['_purchased']	= ( nexusApi::itemIsPurchased( $this->memberData['member_id'], 'downloads', 'file', $r['file_id'] ) );
					}
					elseif ( $r['file_nexus'] )
					{
		...
		$this->output	.= $this->registry->getClass('output')->getTemplate('downloads')->downloadsPortal( $this->canadd, $this->canmod, $category_rows, $feeds, $_recordIds );
	}
	...
	protected function _showIndex()
	{
		...
		$this->output .= $this->registry->getClass('output')->getTemplate('downloads')->indexPage( $this->canadd, $this->canmod, $categoryRows );
	}
