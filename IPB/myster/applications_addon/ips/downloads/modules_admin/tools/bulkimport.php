<?php
/**
 * @file		bulkimport.php 	IP.Download Manager Bulk Import
 *~TERABYTE_DOC_READY~
 * $Copyright: (c) 2001 - 2011 Invision Power Services, Inc.$
 * $License: http://www.invisionpower.com/company/standards.php#license$
 * $Author: bfarber $
 * @since		1st April 2004
 * $LastChangedDate: 2012-02-24 17:47:35 -0500 (Fri, 24 Feb 2012) $
 * @version		v2.5.4
 * $Revision: 10358 $
 */
...
/**
 *
 * @class		admin_downloads_tools_bulkimport
 * @brief		IP.Download Manager Bulk Import
 */
class admin_downloads_tools_bulkimport extends ipsCommand
{
	/**
	...
	public function doExecute( ipsRegistry $registry ) 
	{
		...
		$this->registry->getClass('class_permissions')->checkPermissionAutoMsg( 'idm_bulk' );
	...
	protected function zipFile()
	{
		...
		if( is_dir( $this->settings['upload_dir'] ) ) 
		{
			try
			{
				$classToLoad = IPSLib::loadLibrary( IPSLib::getAppDir('downloads') . '/sources/classes/pclzip.lib.php', 'PclZip', 'downloads' );
		...
		if( is_array($contents) AND count($contents) )
		{
			foreach( $contents as $file )
			{
				//-----------------------------
			 	...
			 	$i = $this->DB->buildAndFetch( array( 'select' => 'record_file_id, record_size', 'from' => 'downloads_files_records', 'where' => "record_realname='" . IPSText::parseCleanValue( $file['filename'] ) . "'" ) );

			 	if( $i['record_file_id'] )
			 	{
				 	$txt				= ( $i['record_size'] == $file['size'] ) ? $this->lang->words['b_yes'] : $this->lang->words['b_maybe'];
				 	$file['exists']		= "<img src='{$this->settings['skin_acp_url']}/images/icons/accept.png' alt='' /> <a href='{$this->settings['board_url']}/index.php?app=downloads&amp;showfile={$i['record_file_id']}' title='{$txt}'><img src='{$this->settings['skin_acp_url']}/images/icons/view.png' alt='{$txt}' /></a>";
				 	$file['is_checked']	= 0;
				 	$checkAll			= false;
			 	}
			 	else
		...
		if( is_dir( $dir ) )
		{
			try
			{
				foreach( new DirectoryIterator( $dir ) as $file )
				{
					...
					{
						$extension	= explode( ".", $file->getFilename() );
						$image		= $this->caches['idm_mimetypes'][ strtolower( array_pop( $extension ) ) ]['mime_img'];
		...
		if( is_array( $files ) )
		{
		 	foreach( $files as $file )
		 	{
				...
				$tempStore	= array(
									...
									'record_link_type'		=> '',
									'record_mime'			=> $this->caches['idm_mimetypes'][ $filetype ]['mime_id'],
				...
				{
					case 'disk':
						$additional_path				= $this->registry->getClass('idmFunctions')->checkForMonthlyDirectory( str_replace( '{root_path}', substr( DOC_IPS_ROOT_PATH, 0, -1 ), $this->settings['idm_localfilepath'] ) );
						
						$tempStore['record_location']	= $additional_path . $tempStore['record_location'];
						
						if( $this->request['remove'] )
						{
							if( !rename( $file, str_replace( '{root_path}', substr( DOC_IPS_ROOT_PATH, 0, -1 ), $this->settings['idm_localfilepath'] ) . "/" . $tempStore['record_location'] ) )
							{
						...
						{
							if( !copy( $file, str_replace( '{root_path}', substr( DOC_IPS_ROOT_PATH, 0, -1 ), $this->settings['idm_localfilepath'] ) . "/" . $tempStore['record_location'] ) )
							{
								$this->registry->output->showError( $this->lang->words['b_nomove'], 11835 );
							}
						}
							
						@chmod( str_replace( '{root_path}', substr( DOC_IPS_ROOT_PATH, 0, -1 ), $this->settings['idm_localfilepath'] ) . "/" . $tempStore['record_location'], IPS_FILE_PERMISSION );
					break;
				...
				$this->DB->insert( 'downloads_files', $tempFile );
				...
				{
					$this->DB->insert( 'downloads_ccontent', array( 'file_id' => $tempFile['file_id'] ) );
				}

				if( $this->settings['idm_filestorage'] == 'db' )
				{
					$this->DB->insert( "downloads_filestorage", array( 	'storage_id'	=> $tempFile['file_id'],
				...
				$this->DB->insert( "downloads_files_records", $tempStore );
	...
	/**
	 * Check if the file is allowed in IP.Downloads
	 *
