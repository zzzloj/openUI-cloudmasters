...
class plugin_nexus_donations implements pluginBlockInterface
{
	/**#@+
	...
	public function getTags()
	{
		return array(
					...
					$this->lang->words['block_plugin_nexus_donations']	=> array(
																		array( '&#36;output', $this->lang->words['block_plugin__donationshtml'] ),
																		),
					);
	}
	...
	public function returnPluginInfo()
	{
		...
		return array(
					'key'			=> 'nexus_donations',
					'name'			=> $this->lang->words['plugin_name__nexus_donations'],
					'description'	=> $this->lang->words['plugin_description__nexus_donations'],
					'hasConfig'		=> false,
					'templateBit'	=> 'block__nexus_donations',
					);
	}
	...
	public function executePlugin( $block )
	{
		...
		require_once( IPSLib::getAppDir( 'nexus' ) . '/sources/hooks.php' );/*noLibHook*/
		$hookGateway	= new nexusHooks( $this->registry );
		$output			= $hookGateway->donationBlock();
