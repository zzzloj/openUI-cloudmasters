<?php
/**
 * @file		plugin_unapprovedrecords.php 	Moderator control panel plugin: show CCS records pending approval
 *~TERABYTE_DOC_READY~
...
/**
 *
 * @class		plugin_ccs_unapprovedrecords
 * @brief		Moderator control panel plugin: show CCS records pending approval
 */
class plugin_ccs_unapprovedrecords
{
	/**
	...
	public function executePlugin( $permissions )
	{
		...
		return $this->registry->getClass('output')->getTemplate('ccs_global')->modcpRecords( $results, $database, $this->databases );
	}
	...
	protected function _getDatabases()
	{
		...
		{
			$this->DB->build( array( 'select' => '*', 'from' => 'ccs_database_moderators', 'where' => 'moderator_approve_record=1' ) );
