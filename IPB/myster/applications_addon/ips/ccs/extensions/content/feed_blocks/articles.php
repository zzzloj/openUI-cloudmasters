...
class feed_articles implements feedBlockInterface
{
	/**#@+
							...
							$this->lang->words['block_feed_articles_db']	=> array(
																				array( '&#36;records', $this->lang->words['block_feed__records'], 
																					array(
																						...
																						array( "&#36;r['image']", $this->lang->words['block_feed__article_image'] ),
	 ...
	 *
	 * @access	public
	 * @return	array 			Plugin data (key (folder name), associated app, name, description, hasFilters, templateBit, inactiveSteps)
	 */
	public function returnFeedInfo()
	{
		...
		return array(
					'key'			=> 'articles',
					'app'			=> 'ccs',
					'name'			=> $this->lang->words['feed_name__articles'],
					'description'	=> $this->lang->words['feed_description__articles'],
					'hasFilters'	=> true,
	...
	public function returnContentTypes( $session = array(), $asHTML = true )
	{
		...
		$_html		= array();
		$_types		= array( array( 'db', $this->lang->words['feed_db__ar'] ), array( 'categories', $this->lang->words['feed_db__categories'] ), array( 'comments', $this->lang->words['feed_db__comments'] ) );
		...
		if( count($this->caches['ccs_databases']) AND is_array($this->caches['ccs_databases']) )
		{
			foreach( $this->caches['ccs_databases'] as $database_id => $database )
			{
				...
				{
					$_input	= "<input type='hidden' name='content_type' value='{$database['database_id']}' />";
					break;
				}
			}
		}
		...
		return array(
					array(
						...
						'description'	=> '',
						'field'			=> $_input . '<ul style="line-height: 1.6"><li>' . implode( '</li><li>', $_html ) . '</ul>',
						),
					);
	}
		...
		if( $_database[1] == 'db' OR !$_database[1] )
		{
			...
			if( is_array($this->caches['ccs_fields'][ $_database[0] ]) AND count($this->caches['ccs_fields'][ $_database[0] ]) )
			{
				foreach( $this->caches['ccs_fields'][ $_database[0] ] as $_field )
				{
					...
					if( !in_array( $_field['field_type'], array( 'input', 'textarea', 'radio', 'select', 'editor', 'date', 'checkbox', 'multiselect' ) ) )
					{
						continue;
					}
	...
	protected function _getRecords( $block, $config, $_database, $previewMode=false )
	{
		...
		{
			$where[]	= "record_locked=" . ( $config['filters']['filter_status'] == 'open' ? 0 : 1 );
		}
		
		if( $config['filters']['filter_visibility'] != 'either' )
		{
			$where[]	= "record_approved=" . ( $config['filters']['filter_visibility'] == 'approved' ? 1 : ( $config['filters']['filter_visibility'] == 'hidden' ? -1 : 0 ) );
		}

		if( $config['filters']['filter_pinned'] != 'either' )
		{
			$where[]	= "record_pinned=" . ( $config['filters']['filter_pinned'] == 'pinned' ? 1 : 0 );
		}
		
		if( $config['filters']['filter_category'] )
		{
			$where[]	= "category_id IN(" . $config['filters']['filter_category'] . ")";
		}
		else
		{
			//-----------------------------------------
			...
			{
				$where[]	= "category_id IN(" . implode( ',', array_keys( $_categories->categories ) ) . ")";
			}
		}
		...
		else if( $config['filters']['filter_starter'] == 'friends' )
		{
			...
			if( count($friends) )
			{
				$where[]	= "member_id IN( " . implode( ',', $friends ) . ")";
			}
			else
			{
				return '';
		...
		{
			$where[]	= "rating_real >= " . $config['filters']['filter_rating'];
		}
		...
		if( count($this->caches['ccs_fields'][ $database['database_id'] ]) AND is_array($this->caches['ccs_fields'][ $database['database_id'] ]) )
		{
			...
			{
				$fields[ $r['field_id'] ]		= $r;
				$fieldsByKey[ $r['field_key'] ]	= $r;
				
				if( $r['field_type'] == 'attachments' )
				{
		...
		/* If not a member, you can't see unpublished content */
		if( !$this->memberData['member_id'] )
		{
			$where[]	= "field_" . $fieldsByKey['article_date']['field_id'] . " <= " . time();
		}
		else
		{
			$_moderate = $dbClass->checkModerator( $database );

			/* If we're not a moderator with edit permission, only show published articles or articles we posted */
			if( !$_moderate['moderate_edit'] )
			{
				$where[]	= "( field_" . $fieldsByKey['article_date']['field_id'] . " <= " . time() . " OR member_id={$this->memberData['member_id']} )";
			}
		}
		...
		$this->DB->build( array(
								'select'	=> '*',
								'from'		=> 'ccs_custom_database_' . $_database[0],
		...
		{
			$dbClass->caches['ccs_attachments_data'][ $_recordId ]	= array();
		}

		if( $_hasAttachments AND count($recordIds) )
		{
			$this->DB->build( array( 'select' => '*', 'from' => 'ccs_attachments_map', 'where' => "map_database_id={$database['database_id']} AND map_record_id IN(" . implode( ',', $recordIds ) . ")" ) );
			$this->DB->execute();
			
			while( $r = $this->DB->fetch() )
			{
				$dbClass->caches['ccs_attachments_data'][ $r['map_record_id'] ][ $r['map_id'] ]	= $r;
			}
		}
	...
	protected function _getComments( $block, $config, $_database, $previewMode=false )
	{
		...
		while( $r = $this->DB->fetch($outer) )
		{
			...
			$r['image']		= $r[ 'field_' . $fieldsByKey['article_image']['field_id'] . '_value' ];
			$r['_isRead']	= $this->registry->classItemMarking->isRead( array( 'catID' => $r['category_id'], 'itemID' => $r['primary_id_field'], 'itemLastUpdate' => $r['record_updated'] ), 'ccs' );
			$r['_database']	= $database;
