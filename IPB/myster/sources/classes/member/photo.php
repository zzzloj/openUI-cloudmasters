...
class classes_member_photo
{
	...
	public function save( $member, $photoType, $gravatar='', $url='' )
	{
		...
		$vkbwOptions = IPSBWOptions::thaw( $member['vk_bwoptions'], 'vkontakte' );
		
		/* Perm checks */
		if ( $photoType != 'facebook' && $photoType != 'twitter' && $photoType != 'vkontakte')
		{
		...
		switch( $photoType )
		{
			...
			break;
			case 'vkontakte':
				$photo = $this->setVkontaktePicture( $member );
				
				if ( $photo['final_location'] )
				{
					$tcbwOptions['tc_s_pic'] = 0;
					$bwOptions['fbc_s_pic']	 = 0;
					$vkbwOptions['vc_s_pic'] = 1;
					$photo['pp_photo_type']  = 'vkontakte';
				}
			break;
		}
		...
		$save = array( 'pp_main_photo'		=> $photo['final_location'],
					   ...
					   'vc_photo'			=> '',
					   'vk_bwoptions'       => IPSBWOptions::freeze( $vkbwOptions, 'vkontakte' ) ); 
	...
	public function remove( $member_id )
	{
		...
		$vkbwOptions			 = IPSBWOptions::thaw( $memberData['vk_bwoptions'], 'vkontakte' );
		$bwOptions['fbc_s_pic']	 = 0;
		$tcbwOptions['tc_s_pic'] = 0;
	...
	public function setVkontaktePicture( $member )
	{
		...
		if ( IPSLib::vkontakte_enabled() && $member['vk_token'] && $member['vk_uid'] )
		{
			...
			if ( $userData['photo'] )
			{
				...
				if ( $mokay === false )
				{
					return array( 'final_location'    => $userData['profile_image_url'],
								  'final_width'       => 50,
								  'final_height'      => 50,
								  't_final_location'  => $userData['profile_image_url'],
								  't_final_width'     => 50,
								  't_final_height'    => 50 );
				}
			}
