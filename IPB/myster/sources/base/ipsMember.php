...
class IPSMember
{
	...
	static public function create( $tables=array(), $autoCreateName=FALSE, $bypassCfields=FALSE, $doStatsRecache=TRUE )
	{
				...
				if( IPSMember::getFunction()->checkNameExists( $finalTables['members']['members_display_name'], array(), 'members_display_name', true ) === true )
				{
					if ( $autoCreateName === TRUE )
					{
						...
						$max = ipsRegistry::DB()->buildAndFetch( array( 'select' => 'MAX(member_id) as max',
														 				'from'   => 'members',
														 				'where'  => "members_l_display_name LIKE '" . ipsRegistry::DB()->addSlashes( IPSText::mbstrtolower( $finalTables['members']['members_display_name'] ) ) . "%'" ) );//-IBR- Мультибайт
			...
			try
			{
				if( IPSMember::getFunction()->checkNameExists( $finalTables['members']['name'], array(), 'name', true ) === true )
				{
					if ( $autoCreateName === TRUE )
					{
						...
						$max = ipsRegistry::DB()->buildAndFetch( array( 'select' => 'MAX(member_id) as max',
														 				'from'   => 'members',
														 				'where'  => "members_l_username LIKE '" . ipsRegistry::DB()->addSlashes( IPSText::mbstrtolower( $finalTables['members']['name'] ) ) . "%'" ) );//-IBR- Мультибайт
		...
		$finalTables['members']['members_l_username']		= isset($finalTables['members']['name']) ? IPSText::mbstrtolower($finalTables['members']['name']) : '';//-IBR- Мультибайт
		...
		$finalTables['members']['members_l_display_name']	= isset($finalTables['members']['members_display_name']) ? IPSText::mbstrtolower($finalTables['members']['members_display_name']) : '';//-IBR- Мультибайт
		$finalTables['members']['fb_uid']	 	            = isset($finalTables['members']['fb_uid']) ? $finalTables['members']['fb_uid'] : 0;
		$finalTables['members']['vk_uid']	 	            = isset($finalTables['members']['vk_uid']) ? $finalTables['members']['vk_uid'] : 0;//-IBR- вконтекте
			...
			else
			{
				if ( $table == 'members' )
				{
					...
					if( isset($data['members_display_name']) AND $data['members_display_name'] )
					{
						...
						$data['members_l_display_name']	= IPSText::mbstrtolower($data['members_display_name']);//-IBR- Мультибайт
					...
					if( isset($data['name']) AND $data['name'] )
					{
						...
						$data['members_l_username']		= IPSText::mbstrtolower($data['name']);//-IBR- Мультибайт
					}
		...
		if ( ! $key_type )
		{
			...
			else
			{
				if ( strstr( $member_key, '@' ) )
				{
					...
					$member_key   = mb_substr( $member_key, 0, 150, strtoupper(IPS_DOC_CHAR_SET)  );//-IBR- Мультибайт, а то имена в 2 раза короче русские будут
		...
		else
		{
			switch( $key_type )
			{
				...
				case 'email':
					if ( is_array( $member_key ) )
					{
						array_walk( $member_key, create_function( '&$v,$k', '$v="\'".( ( strstr( $v, \' \' ) ) ? \'\' : ipsRegistry::DB()->addSlashes( mb_substr( IPSText::mbstrtolower( $v ), 0, 150, strtoupper(IPS_DOC_CHAR_SET) ) ) ) . "\'";' ) );
						$multiple_ids = $member_key;
					}
					else
					{
				...
				case 'username':
					if ( is_array( $member_key ) )
					{
						array_walk( $member_key, create_function( '&$v,$k', '$v="\'".ipsRegistry::DB()->addSlashes( mb_substr( IPSText::mbstrtolower( $v ), 0, 255, strtoupper(IPS_DOC_CHAR_SET) ) ) . "\'";' ) );//-IBR- Мультибайт
						$multiple_ids = $member_key;
					}
					else
					{
						$member_key   = mb_substr( $member_key, 0, 255, strtoupper(IPS_DOC_CHAR_SET) );//-IBR- Мультибайт 
						$member_value = "'" . ipsRegistry::DB()->addSlashes( IPSText::mbstrtolower( $member_key ) ) . "'"; //-IBR- Мультибайт
					}
	...
	static public function buildNoPhoto( $member, $size=null, $noCache=false, $noBorder=false )
	{
		...
		if ( 1 == 2 and ipsRegistry::$settings['allow_gravatars'] and ! $member['bw_disable_gravatar'] ) //no gravatar default photo caching //-IBR- отключаем кешинг дефолтной аватарки по просьбам трудящихся. С ним связан ряд багов.
		{
